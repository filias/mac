...
def detail(request, artist_id):
    ...
    return render(request, 'artistas/templates/artistas_detalhe.html', {'artista': artista, 'obras': obras, 'telas': telas, 'exposicoes': exposicoes, 'imprensa': imprensa, 'criticas': criticas,  'acervo':acervo})

def obras(request, artist_id):
    artista = get_object_or_404(Artista, pk=artist_id)
    obras = Obra.objects.filter(autor=artist_id, estado__in=['C', 'E']).order_by('-ano')
    telas = artist_canvas(artista)
    return render(request, 'artistas/templates/artistas_obras.html', {'artista': artista, 'obras': obras, 'telas': telas})

def acervo(request, artist_id):
    artista = get_object_or_404(Artista, pk=artist_id)
    obras = Obra.objects.filter(autor=artist_id, estado='A').order_by('-ano')
    telas = artist_canvas(artista)
    return render(request, 'artistas/templates/artistas_acervo.html', {'artista': artista, 'obras': obras, 'telas': telas})

def exposicoes(request, artist_id):
    artista = get_object_or_404(Artista, pk=artist_id)
    exposicoes = Exposicao.objects.filter(artistas__id__exact=artista.id).order_by('-data_inicio')
    telas = artist_canvas(artista)
    return render(request, 'artistas/templates/artistas_exposicoes.html', {'artista': artista, 'exposicoes': exposicoes, 'telas': telas})

def critica(request, artist_id):
    artista = get_object_or_404(Artista, pk=artist_id)
    criticas = Publicacao.objects.filter(artista__id__exact=artista.id, tipo='Critica').order_by('-data')
    telas = artist_canvas(artista)
    return render(request, 'artistas/templates/artistas_critica.html', {'artista': artista, 'criticas': criticas, 'telas': telas})

def imprensa(request, artist_id):
    artista = get_object_or_404(Artista, pk=artist_id)
    imprensa = Publicacao.objects.filter(artista__id__exact=artista.id, tipo='Imprensa').order_by('-data')
    telas = artist_canvas(artista)
    return render(request, 'artistas/templates/artistas_imprensa.html', {'artista': artista, 'imprensa': imprensa, 'telas': telas})
...
def obra_detalhe(request, obra_id,  artist_id):
    ...
    return render(request, 'artistas/templates/obra_detalhe.html', {'obra': obra, 'telas': telas, 'tecnicas': tecnicas, 'materiais': materiais})
