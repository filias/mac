...
def index(request):
    RTR_DICT["monografias"] = Publicacao.objects.filter(tipo="Monografia").order_by(
        "-data"
    )[:3]
...
def newsletters(request, select):
    RTR_DICT["publicacoes"] = Publicacao.objects.filter(
        tipo="Newsletter", artista__nome__range=(select[0], select[1])
    ).order_by("artista")
    return render(
        request, "publicacoes/templates/publicacoes_newsletters.html", RTR_DICT
    )
...
def monografias(request, select):
    RTR_DICT["publicacoes"] = Publicacao.objects.filter(
        tipo="Monografia", artista__nome__range=(select[0], select[1])
    ).order_by("artista")
    return render(
        request, "publicacoes/templates/publicacoes_monografias.html", RTR_DICT
    )
